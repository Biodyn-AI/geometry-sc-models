...
# Works from either tree: the study directory (manifolds/) or the public repository, whichever
# this file happens to be sitting in. Every number plotted comes from a committed JSON.
_HERE = os.path.dirname(os.path.abspath(__file__))
_ROOT = os.path.dirname(_HERE)
_CANDIDATES = [(os.path.join(_ROOT, "synthetic", "results"),
                os.path.join(_ROOT, "gaps", "results")),
               (os.path.join(_ROOT, "experiments", "05_synthetic", "results"),
                os.path.join(_ROOT, "experiments", "06_gap_tests", "results"))]
SYN, GAP = next((s, g) for s, g in _CANDIDATES if os.path.isdir(s))
