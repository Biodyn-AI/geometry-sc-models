...
\textbf{E} A worked negative: the extracted-operator battery
...
\textbf{Construct caveat.} There is no cell vector in any of these
...
\textbf{Circularity.} \(\varphi\) and the phase call are \emph{derived from marker expression}, so the
...
66/180, which an earlier k \(\le\) 50 cap concealed.
...
\hypertarget{d.4-why-the-steering-arm-is-excluded}{%
\subsection{D.4 Why the steering arm is excluded}\label{d.4-why-the-steering-arm-is-excluded}}
...
\hypertarget{d.5-the-stall-theorem}{%
\subsection{D.5 The stall theorem}\label{d.5-the-stall-theorem}}
...
\textbf{Why the global quadratic failed:} Q's own gradient field rotates only
